...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `--detector` | pyscene | 检测方式：`pyscene` 三检测器并用；`ffmpeg` 用 ffmpeg 场景滤镜，快约 3.6 倍 |
| `--threshold` | 24.0 | 内容检测阈值，越小越敏感、切得越碎（PySceneDetect 常规默认 27） |
| `--min-scene-len` | 0.6 | 最短镜头时长（秒），短于此的段会并入相邻镜头 |
| `--aggressive` | 关闭 | 激进模式，各检测器灵敏度大幅调高，几乎不漏切但切得更碎（仅 pyscene 有效） |

> ⚠️ **`--detector ffmpeg` 快但明显不准，不建议用于数据集生产。**
> 在 6 个真实视频（合计 70 分钟）上实测：它只检出了 pyscene 约 1/4 的镜头（190 vs 811），
> 导致 **65% 的导出片段内部含有未被切开的镜头切换**。调低阈值也救不回来
> （阈值从 0.24 降到 0.10，召回率仅从 29% 升到 38%，污染率仍有 62%）。
> 它快是因为少干了四分之三的活。
...
**产出校验**：每个片段导出后都会验证文件非空且能被 ffprobe 解析，不合格的一律删除并判为失败，
不写进 `scene.json`。断点续跑（`--skip-existing`）时也会校验已存在的文件，无效的删除后重做。

这层保护不是可有可无的：ffmpeg 会**先创建输出文件再初始化编解码器**，一旦初始化失败
（GPU 掉线、驱动故障、会话耗尽）就会留下 0 字节文件。没有校验的话，这些空文件会被
记进 `scene.json` 当作"已完成"，之后被 `--skip-processed` 永久跳过——成为再也不会被发现的坏样本。
...
**NVENC 并发上限**：消费级显卡（GeForce 系列）的 NVENC 编码会话上限是**整机 8 路**，
多插一张卡也不会叠加（实测双卡轮询仍是 8/12 失败）。所以 GPU 模式下 `--jobs` 不要超过 8。

**故障保护**：ffmpeg 调用失败时按原因分别处理——NVENC 会话耗尽会退避重试，
识别到 GPU 掉线/驱动故障（`cuInit failed`、`CUDA_ERROR` 等）则立即降级到 CPU 编码而不做无谓重试。

### 分布式分片

| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `--shard` | 无 | 形如 `i/N`，只处理属于第 i 片的视频（共 N 片） |

多台机器分工时，每台跑同一条命令、只改分片号即可：

```bash
python split_shots.py ./videos ./out --shard 0/4    # 节点 1
python split_shots.py ./videos ./out --shard 1/4    # 节点 2 ...
```

分片依据是**视频相对路径的 SHA-256 取模**，各节点无需任何通信就能算出同一份划分，
天然不重不漏。配合 `--skip-processed` 可以随时增删节点重跑。
